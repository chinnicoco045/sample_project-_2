...
test("handleUpdate is called when id exists and is not 'multiProduct'", async () => {
  mockUseParams.id = '123'; // Set id to a valid value that is not 'multiProduct'

  const putSpy = jest.spyOn(service, 'put').mockResolvedValueOnce({
    data: { status: 200 },
  });
  ...
  const contextValue = {
    ...mockRowContextValues,
    selectedRow: [{ productName: 'Test Product' }],
    setChecked: setCheckedMock,
